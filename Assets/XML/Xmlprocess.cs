...
public class Xmlprocess
{
   ...
   /* private Boolean isExits()
    ...
    {
        if(System.IO.File.Exists(path + _FileName))//如果已經有xml檔，讀XML檔
        {

        }else if (isExits())//如果沒有xml檔，讀SQL
        {
            XmlNode node = xmlDoc.SelectSingleNode("Loadfile/User");
            XmlElement element = (XmlElement)node;
            XmlAttribute[] attributeList = { element.GetAttributeNode("ID"), element.GetAttributeNode("name"), element.GetAttributeNode("level"), element.GetAttributeNode("sex") };
    ...
    */
    ...
    /// 新增每回單字學習紀錄
    /// </summary>
    public void createPracticeRecord()
    {
        XmlNode nodeLast = null;
        XmlElement practice_history = null;
        ...
        if (isExits())
        {
            ...
            if (attributeLast.Value.ToString() != DateTime.Now.ToString("yyyy-MM-dd"))//如果最近一筆紀錄不是今天的日期
            {
                ...
                XmlElement practicehistory = xmlDoc.CreateElement("practice_history"); ;
                log_record.AppendChild(practicehistory);
                practice_history = practicehistory;
            }
            else
            {
                XmlNode n_practice_history = nodeLast.SelectSingleNode("practice_history");//在最近一筆的log下。找到learning節點
                practice_history = (XmlElement)n_practice_history;
            }

            XmlElement practice_record = xmlDoc.CreateElement("practice_record");
            practice_history.AppendChild(practice_record);

            practice_record.SetAttribute("theme", "");
            practice_record.SetAttribute("level", "");
            practice_record.SetAttribute("startTime", DateTime.Now.ToString("HH: mm:ss"));
            practice_record.SetAttribute("score", "0");
            practice_record.SetAttribute("correct", "0");
            practice_record.SetAttribute("maxcorrect", "0");
            practice_record.SetAttribute("endTime", "");
    ...
    public void setPracticeTypeScoreRecord(string TypeName,int score)
    {
        if (isExits())
        {
            XmlNode nodeLastLearning = null;
            XmlNodeList nodelist_Previous = xmlDoc.SelectNodes("//practice_record");
            ...
            XmlAttribute attr_optionscore = element.GetAttributeNode("score");
    ...
    public string[] setPracticeInfo(string theme,string level)
    {
        if (isExits())
        {
            ...
            nodelist_Previous = xmlDoc.SelectNodes("//practice_record");
            foreach (XmlNode itemsNode in nodelist_Previous)
            {
                XmlAttributeCollection xAT2 = itemsNode.Attributes;
                for (int j = 0; j < xAT2.Count; j++)
    ...
    /// 更新每回單字學習紀錄的分數與結束時間
    /// </summary>
    public string[] setPracticeScoreRecord(int score)
    {
        if (isExits())
        {
            XmlNode nodeLastLearning = null;
    ...
    /// 更新該回單字學習紀錄的正確與錯誤題數
    /// </summary>
    public string setPracticeCorrect(int correctCount,int wrongCount)
    {
        if (isExits())
        {
            /*更新學習紀錄的答對題數*/
    ...
    /// <summary>
    /// 更新每回單字學習紀錄的最高連續正確題數 leaening_task
    /// </summary>
